...
function myFunction() {
	...
	document.getElementById("answer").innerHTML="Your input is blank";      
		...
		||inputText.toLowerCase().indexOf("pay")>-1&&inputText.toLowerCase().indexOf("minister")>-1){
			...
			document.getElementById("answer").innerHTML=`My earliest and most vivid recollection is of being held by my ears over a well in the compound of a house where my family was then living, at what is now Tembeling Road in Singapore. I was about 4 years old. I had been mischievous and had messed up an expensive jar of my father's 4 71 1 pale green scented brilliantine. My father had a violent temper, but that evening his rage went through the roof. He took me by the scruff of the neck from the house to this well and held me over it. How could my ears have been so tough that they were not ripped off, dropping me into that well? Fifty years later, in the 1970s, I read in the Scientific American an article explaining how pain and shock released neuropeptides in the brain, stamping the new experience into the brain cells and thus ensuring that the experience would be remembered for a long time afterwards. I was born in Singapore on 16 September 1923, in a large two-storey bungalow at 92 Kampong Java Road. Memoirs of LKY,1998`;
		}
		else if(inputText.toLowerCase().indexOf("parents")>-1){
		...
		else if(inputText.toLowerCase().indexOf("your father")>-1||inputText.toLowerCase().indexOf("father's education")>-1){
			document.getElementById("answer").innerHTML=`He was educated in English at St Joseph's Institution, a Catholic mission school founded by the De La Salle Brothers in 1852. He said he 25 completed his Junior School Certificate, after which he ended his formal education - to his and my mother's eternal regret. Being without a profession, he could only get a job as a storekeeper with the Shell Oil Company when the fortunes of both families were destroyed in the Great Depression. Memoirs of LKY,1998`;
		}
		else if(inputText.toLowerCase().indexOf("grandfather")>-1||inputText.toLowerCase().indexOf("xx")>-1){
